...
class EnhancedGTDRequestHandler(BaseHTTPRequestHandler):
...
<body>
    ...
    def serve_gtd_tasks(self):
        """Serve the tasks.md file content with enhanced progress support"""
        try:
            ...
            if 'application/json' in accept_header:
                # Parse markdown to JSON with progress support
                tasks = self.parse_markdown_to_json_with_progress(content)
    ...
    def parse_markdown_to_json_with_progress(self, markdown):
        """Parse markdown tasks file to JSON structure with progress and notes support"""
        tasks = {
            'projects': [],
            'next_actions': [],
            'waiting_for': [],
        ...
        while i < len(lines):
            ...
            if line == '# Projects':
                ...
                # Check for progress indicator (e.g., [50%])
                progress_match = re.search(r'\[(\d+)%\]', actual_text)
                if progress_match:
                    progress = int(progress_match.group(1))
                    # Remove progress indicator from text
                    actual_text = re.sub(r'\s*\[\d+%\]\s*', ' ', actual_text).strip()
                
                # Check for notes in next lines (indented)
                notes_lines = []
                j = i + 1
                while j < len(lines) and lines[j].startswith('  '):
                    notes_lines.append(lines[j].strip())
                    j += 1
                
                if notes_lines:
                    notes = '\n'.join(notes_lines)
                    i = j - 1  # Skip the notes lines
                
                tasks[current_category].append({
                    'text': actual_text,
                    'completed': completed,
                    'progress': progress,
                    'notes': notes
    ...
    def add_gtd_task(self):
        """Add a new task with progress and notes support"""
        try:
    ...
    def update_gtd_tasks(self):
        """Update the entire tasks.md file with progress and notes support"""
        try:
            content_length = int(self.headers.get('Content-Length', 0))
            new_content = self.rfile.read(content_length).decode('utf-8')
...
def run(port=8081):
    server_address = ('', port)
    httpd = ThreadedHTTPServer(server_address, EnhancedGTDRequestHandler)
    print(f"Starting enhanced GTD web server on port {port}...")
